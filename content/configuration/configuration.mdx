...
Four styles ship in `src/theme/themes.ts`, each with light + dark:
`steel` (the default), `aurora`, `retro`, `shadcn`. Users switch via the gear
menu; pin one with `VITE_DEFAULT_THEME_STYLE` to lock branding for a public site.
